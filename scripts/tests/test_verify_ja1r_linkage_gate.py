...
f82c9bd4, tools/solstone-macos-gate/gate.py: new_report() (direct-lane shape,
...
def spl_link_report(sol_dmg_sha256=DEFAULT_SOL_DMG_SHA, run_id=RUN_ID):
    # Schema-derived from extro-tools f82c9bd4, not captured live evidence.
    return {
        "result": "PASS",
        "run_id": run_id,
...
def report_for(filename, sol_dmg_sha256=DEFAULT_SOL_DMG_SHA):
    ...
    if filename in ("fresh-acquire.json", "discovered-adopt.json"):
        # Derived from gate.py at f82c9bd4: new_report() scenario fields for the
    ...
    if filename == "journal-upgrade.json":
        # Derived from gate.py at f82c9bd4: new_report(),
