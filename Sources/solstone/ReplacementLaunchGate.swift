...
public struct ReplacementLaunchCommand: Sendable, Equatable {
    let predecessorPID: Int32
    let bundlePath: String
    let shellCommand: String
}

enum ReplacementLaunchGate {
    ...
    static func command(predecessorPID: Int32, bundlePath: String) -> ReplacementLaunchCommand {
        let quotedBundlePath = singleQuotedShellArgument(bundlePath)
        let command = "i=0; while [ $i -lt \(maxPolls) ] && kill -0 \(predecessorPID) 2>/dev/null; do i=$((i+1)); sleep \(shellSeconds(pollInterval)); done; /usr/bin/open -n \(quotedBundlePath)"
        return ReplacementLaunchCommand(
    ...
    static func runDetached(_ command: ReplacementLaunchCommand) throws {
        let process = Process()
    ...
    /// Unit-tested behavioral spec for the detached shell emitted by `command(predecessorPID:bundlePath:)`.
    /// Production launches the shell; this function keeps the polling/launch semantics fake-testable.
