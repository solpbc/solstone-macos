...
struct AXIDTests {
    ...
    @Test func enumTokensAreExactAndMatchTokenGrammar() {
        ...
        expectTokensMatchGrammar(SetupGroupVerdictAXState.allCases.map(\.axToken))
        expectTokensMatchGrammar(LastJournalDeliveryAXState.allCases.map(\.axToken))
        expectTokensMatchGrammar(LastJournalContactAXState.allCases.map(\.axToken))
